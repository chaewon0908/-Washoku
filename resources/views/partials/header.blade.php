    ...
    <div class="container mx-auto px-4 relative z-10">
            ...
            <div class="flex items-center gap-3">
                ...
                @if(auth()->guard()->check())
                    ...
                    <div class="flex items-center gap-3 md:gap-4">
                        <a href="{{ route('dashboard') }}" class="flex items-center gap-2 text-sm md:text-base font-medium text-white/90 hover:text-amber-300 transition-colors group">
                        ...
                        <a href="{{ route('dashboard') }}" class="text-sm font-medium text-white/70 hover:text-amber-300 transition-colors">
                            Profile
                        </a>
                ...
                @else
                    ...
                    <div class="flex items-center gap-2 md:gap-3">
                        <a href="{{ route('login') }}" class="flex items-center gap-1.5 md:gap-2 text-sm font-medium text-white/80 hover:text-white transition-colors px-3 py-2 rounded-full hover:bg-white/5 border border-transparent hover:border-white/10">
                            <svg class="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1"/>
                            </svg>
                            <span class="hidden sm:inline">Log In</span>
                        </a>
                        <a href="{{ route('register') }}" class="relative overflow-hidden bg-gradient-to-r from-white/10 to-white/5 backdrop-blur-md px-5 py-2.5 rounded-full text-sm font-semibold transition-all duration-300 border border-white/20 hover:border-amber-400/40 group">
        ...
        <nav class="border-t border-white/10">
            <ul class="flex gap-1 lg:gap-2 py-3 items-center overflow-x-auto scrollbar-hide">
                ...
                @if(auth()->guard()->check())
                <li>
                    <a href="{{ route('dashboard.orders') }}" class="nav-link flex items-center gap-2 {{ request()->routeIs('dashboard.orders') ? 'nav-link-active' : '' }}">
                ...
                @if(Auth::check() && (Auth::user()->isAdmin() || Auth::user()->isOrderProcessor()))
                <li class="ml-auto">
                    <a href="{{ route('admin.index') }}" class="inline-flex items-center gap-2 py-2.5 px-5 rounded-full font-bold text-white bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 transition-all duration-300 shadow-lg hover:scale-105">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
