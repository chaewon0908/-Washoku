                        ...
                        <tr class="hover:bg-gradient-to-r hover:from-gray-50/50 hover:to-white/50 transition-all duration-200 reveal-scale" style="transition-delay: {{ $loop->index * 30 }}ms;">
                            ...
                            <td class="py-4 px-6">
                                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold shadow-sm
                                    ...
                                    @elseif($order->status === 'delivering') bg-gradient-to-r from-indigo-100 to-indigo-50 text-indigo-700 border border-indigo-200
                                    @elseif($order->status === 'completed') bg-gradient-to-r from-green-100 to-green-50 text-green-700 border border-green-200
                                    @else bg-gradient-to-r from-red-100 to-red-50 text-red-700 border border-red-200
                                    @endif">
                            ...
                            <td class="py-4 px-6">
                                <form method="POST" action="{{ route('admin.orders.update-status', $order) }}" class="inline-block">
                                    ...
                                    <select name="status" onchange="this.form.submit()" class="px-3 py-2 text-sm font-semibold rounded-lg border-2 transition-all duration-200 shadow-sm hover:shadow-md
                                        ...
                                        @elseif($order->status === 'delivering') bg-gradient-to-r from-indigo-50 to-indigo-100/50 text-indigo-700 border-indigo-300 hover:border-indigo-400
                                        @elseif($order->status === 'completed') bg-gradient-to-r from-green-50 to-green-100/50 text-green-700 border-green-300 hover:border-green-400
                                        @else bg-gradient-to-r from-red-50 to-red-100/50 text-red-700 border-red-300 hover:border-red-400
                                        @endif">
