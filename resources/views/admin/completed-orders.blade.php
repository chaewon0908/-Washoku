    ...
    <div class="container mx-auto px-4 sm:px-6 relative z-10">
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-xl overflow-hidden border border-gray-100 reveal" x-data="{ searchQuery: '{{ request('search') }}' }">
            ...
            <div class="p-4 sm:p-6 border-b border-gray-100 bg-gradient-to-r from-green-50/50 to-white">
                <form method="GET" action="{{ route('admin.orders.completed') }}" class="flex flex-col sm:flex-row gap-3 sm:gap-4">
                    ...
                    <div class="flex gap-2">
                        ...
                        @if(request('search'))
                        <a href="{{ route('admin.orders.completed') }}" class="px-4 sm:px-6 py-2.5 sm:py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm sm:text-base font-semibold rounded-lg sm:rounded-xl transition-all flex items-center gap-1.5 sm:gap-2">
                            <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                            </svg>
                ...
                <table class="w-full min-w-[800px]">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gradient-to-r hover:from-green-50/30 hover:to-white/50 transition-all duration-200 reveal-scale" style="transition-delay: {{ $loop->index * 30 }}ms;">
                            ...
                            <td class="py-3 sm:py-4 px-3 sm:px-6">
                                <span class="text-sm sm:text-lg font-bold text-green-600">₱{{ number_format($order->total, 2) }}</span>
                            </td>
                            <td class="py-3 sm:py-4 px-3 sm:px-6">
